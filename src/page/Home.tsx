	...
	return (
		<div className='h-full w-full'>
			...
			<div className='h-20 w-full flex items-center justify-around'>
				<div
					onClick={() => changePages()}
					className={`bg-[#2c2c2c] h-10 w-44 md:w-52 lg:w-64 flex items-center justify-center rounded-md cursor-pointer ${processing && 'border-[1px] border-brand-500'}`}
				>
					<p className={`text-[#5e5c5c] font-medium ${processing && 'text-brand-500 font-light'}`}>Em processo</p>
				</div>
				<div
					onClick={() => changePages()}
					className={`bg-[#2c2c2c] h-10 w-44 md:w-52 lg:w-64 flex items-center justify-center rounded-md cursor-pointer ${completed && 'border-[1px] border-brand-500'}`}
				>
					<p className={`text-[#5e5c5c] font-medium ${completed && 'text-brand-500 font-light'}`}>Finalizadas</p>
				</div>
			</div>
			<div className='h-[55vh] w-full flex items-center justify-start flex-col overflow-scroll overflow-x-hidden scrollbar-thumb-zinc-700 scrollbar-track-transparent scrollbar-thin'>
				{processing && pages?.map((page: Page) => {
					if (page.phase === 'processing') {
						return (
